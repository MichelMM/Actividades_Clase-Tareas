...
boolean graph_addVertex(Graph graph, Type data) {
	for (unsigned long i = 0; i < graph->num_vertex; i++) {
	...
	unsigned long size = graph->num_vertex - 1;
	graph->array = (NodeG*) realloc(graph->array,
			sizeof(struct strNodeG) * graph->num_vertex);
...
//	((*graph).array[size]) = new;
	...
	//printf("Vertices: %lu",graph->num_vertex);
	return true;
}
